...
from model_pytorch import GCN_reimp
...
def gae_for(args, position):
    ...
    #     features_all_pre)
    # features_pre = np.load(
    #     '/media/chundi/3b6b0f74-0ac7-42c7-b76b-00c65f5b3673/revisitop/cnnimageretrieval-pytorch/data/test/matlab_data/features_pre.npy')
    # features_all_pre = np.load(
    #     '/media/chundi/3b6b0f74-0ac7-42c7-b76b-00c65f5b3673/revisitop/cnnimageretrieval-pytorch/data/test/matlab_data/features_all_pre.npy')
    n_nodes, feat_dim = features_pre.shape

    features_pre_tensor = torch.FloatTensor(features_pre)
    features_all_pre_tensor = torch.FloatTensor(features_all_pre)

    cudnn.benchmark = True
    model = GCN_reimp(feat_dim, args.hidden1).cuda()
    #model = torch.nn.DataParallel(model).cuda()
    optimizer = optim.Adam(model.parameters(), lr=args.lr, betas=(0.9, 0.999), eps=1e-3, weight_decay=1e-5)
    #optimizer = optim.SGD(model.parameters(), lr=args.lr, momentum=0.99)

    input_var = torch.autograd.Variable(features_pre_tensor).cuda()
    inference_var = torch.autograd.Variable(features_all_pre_tensor).cuda()

    #labels = torch.FloatTensor(adj_label.toarray()).cuda()
    #labels = torch.FloatTensor(np.random.randn(features_pre.shape[0], features_pre.shape[0])).cuda()
    #labels.requires_grad = True
    #input_var.requires_grad = True
    labels = torch.empty([features_pre.shape[0], features_pre.shape[0]])
    labels = torch.nn.init.normal_(labels, 0, 1)
    labels = torch.autograd.Variable(labels).cuda()
    print(labels.requires_grad)
    pos_weight = torch.from_numpy(np.array(pos_weight, dtype=np.float32))
    #norm = 1.0 / features_pre.shape[0]
    best_revop = 0
    print('norm: {}'.format(norm))
    for epoch in range(args.epochs):
        model.train()
        start_time = time.time()
        adj_preds, _ = model(input_var)
        loss = norm * F.binary_cross_entropy_with_logits(adj_preds, adj_preds, pos_weight=pos_weight)
        #loss = norm * F.mse_loss(adj_preds, labels.cuda())
        ...
        model.eval()
        start_time = time.time()
        _, hidden_emb = model(inference_var)
        hidden_emb_np = hidden_emb.cpu().data.numpy()
        revop_map = get_roc_score_matrix(hidden_emb_np, Q.shape[1])
        eval_end_time = time.time() - start_time
        if revop_map > best_revop:
            best_revop = revop_map
        print("Iter: {}, train loss: {}, train time: {}, eval time: {}, revop:{}, best revop:{}".format(epoch, loss.item(), str(train_end_time), str(eval_end_time), revop_map,
                                                                                   best_revop))
